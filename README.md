...
| Process | `starting-and-momentum` | Overcoming procrastination and perfectionism in growth work — the dated action plan, head-start trick, deliberately weak first draft, alternative entry point, "just until" micro-goals, the 60-minute pact, small compounding steps, and when stalling is structural rather than psychological |
| Process | `analytics-and-tracking` | Tool categories and when each is needed, the tracking plan and event taxonomy, identity resolution, server-side versus client-side collection, consent effects on the data, validation and reconciliation |
